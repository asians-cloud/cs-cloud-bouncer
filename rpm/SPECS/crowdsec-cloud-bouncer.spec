...
Summary:      Cloud bouncer for Crowdsec 
...
%global name crowdsec-cloud-bouncer
%global __mangle_shebangs_exclude_from /usr/bin/env

%prep
%setup -n crowdsec-cloud-bouncer-%{version}
...
TMP=`mktemp -p /tmp/`
cp /etc/crowdsec/bouncers/crowdsec-cloud-bouncer.yaml ${TMP}
API_KEY=${API_KEY} envsubst < ${TMP} > /etc/crowdsec/bouncers/crowdsec-cloud-bouncer.yaml
rm ${TMP}
...
echo "please enter the binary path in '/etc/crowdsec/bouncers/crowdsec-cloud-bouncer.yaml' and start the bouncer via 'sudo systemctl start crowdsec-cloud-bouncer' "
...
if [ "$1" == "0" ] ; then
    systemctl stop crowdsec-cloud-bouncer || echo "cannot stop service"
    systemctl disable crowdsec-cloud-bouncer || echo "cannot disable service"
fi
...
if [ "$1" == "1" ] ; then
    systemctl restart  crowdsec-cloud-bouncer || echo "cannot restart service"
elif [ "$1" == "0" ] ; then
    systemctl stop crowdsec-cloud-bouncer
    systemctl disable crowdsec-cloud-bouncer
fi
